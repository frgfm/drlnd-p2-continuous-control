...
class Actor(nn.Module):
    ...
    def __init__(self, state_size, action_size, fc_units=128, bn=True, dropout_prob=0.):

        super(Actor, self).__init__()

        # Number of nodes in FC layers
        lin_features = [state_size, fc_units, fc_units, action_size]
        ...
        for idx, out_feats in enumerate(lin_features[1:]):
            layers.extend(make_layer(lin_features[idx], out_feats,
                                     ...
                                     bn if idx + 2 < len(lin_features) else False,
...
class Critic(nn.Module):
    ...
    def __init__(self, state_size, action_size, fc_units=64, bn=False, dropout_prob=0.):

        super(Critic, self).__init__()

        # Number of nodes in FC layers
        base_feats, head_feats = [state_size, fc_units], [fc_units + action_size, fc_units, 1]
        ...
        for idx, out_feats in enumerate(head_feats[1:]):
            layers.extend(make_layer(head_feats[idx], out_feats,
                                     ...
                                     bn if idx + 2 < len(head_feats) else False,
